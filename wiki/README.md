...
| [tools/audit-choix.js](../tools/audit-choix.js) | **Choice audit.** `node tools/audit-choix.js`. The content checker says whether a word exists; this says whether a *choice* exists. Reports free options whose every effect is negative and which buy nothing durable (a trait, a chain, a flag, a position), options strictly dominated by another free option, and — the most useful figure — how many options each background, temperament and origin unlocks. That last one caught the real problem: `hardworking` opened one option in the whole game against thirty-four for `calculating`, so picking a temperament changed almost nothing. Exits non-zero when a dead or dominated option is left. `--profils` prints only the unlock table |
