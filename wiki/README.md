...
| [tools/audit-choix.js](../tools/audit-choix.js) | **Choice audit.** `node tools/audit-choix.js`. The content checker says whether a word exists; this says whether a *choice* exists. Six measures. **Dead options**: free options whose every effect is negative and which buy nothing durable. **Dominated options**: strictly worse than another free option. **The option that wins whatever you value**: eight hundred random weightings over the game's currencies (one player plays popularity, another standing, another money, another energy); an option winning under 80 % of them is not a choice, it is the answer. The usual culprit is not an overpowered option but a *cautious option that gives up nothing* — it gains on five axes and cedes none, while the risky one is a gamble whose success barely beats that certainty. **What feeds a roll**: a roll reading one stat rewards one profile; background, traits and situation should weigh on the odds, not only open options. **The balance of power**: how many branches move it, and how many move it *against* the career or to a rival camp's benefit — that second figure is what makes a choice an arbitration rather than a reward. **What each profile unlocks**: options gated per background, temperament and origin. Exits non-zero on a dead or dominated option. `--profils` prints only the unlock table |
| [tools/audit-stats.js](../tools/audit-stats.js) | **Stat audit.** `node tools/audit-stats.js`. Looks at the *sheet*: what the eight stats are worth, where they come from, and whether they still mean anything at fifty. Four measures. **What the content offers** — points given and taken back per stat, across every deck. **What is read against what is paid** — a stat feeds rolls and opens gates; one paid a lot and never read is a currency with nothing to buy (notoriety: 864 points given, five rolls read it), one read a lot and never paid is a birth endowment you cannot build on (charisma: 124 readings, 14 points in the whole game). **What a career actually banks** — whole careers played, points offered against points that land, with the ratio to the twenty-point scale. **The curve by age** — where the eight bars sit at 35, 40, 45…, and how many are full; this is the one that answers *at what age does the sheet stop moving?* The pilot plays at random, so every figure is a floor: a player who chooses well banks more and saturates earlier. `--contenu` skips the careers; `PM_CARRIERES` tunes the run. It never says the game is too easy or too hard — that is regression.js's trace. |
