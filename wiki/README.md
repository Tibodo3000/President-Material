...
| File | Contains |
|------|----------|
| [js/events/](../js/events/) | **376 events** across 7 decks (`events`, `campaign`, `runoff`, `nomination`, `support`, `aside`, `races`). Split by theme into 14 files (`debuts`, `medias`, `argent`, `appareil`, `arbitrages`, `chaines`, `assemblee`, …) plus one file per auxiliary deck, all reassembled into `EVENT_DATA` by `_assemble.data.js`. The engine still reads a single `EVENT_DATA`. |
